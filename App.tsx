           ...
           <div className="flex flex-col h-full overflow-hidden bg-slate-50 rounded-xl border border-slate-200 p-4">
              {sectionType === 'READING' ? (
                ...
                <div className="flex-1 flex flex-col h-full min-h-[500px]">
                   ...
                   <TextArea 
                     className="flex-1 w-full p-6 border border-slate-200 rounded-xl bg-slate-100 text-slate-500 resize-none font-mono text-base leading-relaxed cursor-not-allowed"
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="bg-slate-900 text-white px-6 py-4 flex justify-between items-center">
        ...
        <div className="font-mono text-xl font-bold bg-slate-800 px-4 py-2 rounded text-yellow-400">
          {formatTime(timeLeft)}
        </div>
        <Button variant="secondary" size="sm" onClick={onExit}>Exit Test</Button>
      </header>
        ...
        <div className="w-1/2 p-8 overflow-y-auto border-r border-slate-200 bg-white">
          <div className="prose max-w-none text-slate-800">
             {currentPart.instructions && (
               <div className="bg-blue-50 p-4 rounded-lg mb-6 text-sm text-blue-800 font-medium border border-blue-100">
                 {currentPart.instructions}
               </div>
             )}
             ...
             <div className="space-y-6 max-w-xl mx-auto">
               {currentPart.questions.map((q, idx) => {
                 if (q.type === 'PASSAGE') {
                   ...
                   return (
                     <div key={q.id} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
                        <div className="leading-relaxed text-slate-800">
                          {parts.map((txt, i) => {
                             // If it's a number, it's a placeholder ID
                             if (i % 2 === 1) { 
                               // Find corresponding CLOZE question
                               const clozeQ = currentPart.questions.find(cq => cq.type === 'CLOZE' && cq.text === txt);
                               if (!clozeQ) return <span key={i} className="text-red-500 font-bold">[{txt}?]</span>;
                               
                               return (
                                 <select 
                                   key={i}
                                   className="mx-1 border-b-2 border-blue-400 bg-blue-50 py-0.5 px-2 text-blue-900 font-bold focus:outline-none rounded-t"
                                   value={answers[clozeQ.id] || ''}
                                   onChange={e => setAnswers(prev => ({...prev, [clozeQ.id]: e.target.value}))}
                                 >
                                    <option value="">Select...</option>
                                    {clozeQ.options?.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                 </select>
                               );
                             }
      ...
      <footer className="bg-white border-t border-slate-200 p-4 flex justify-end">
        ...
        {activeTab === 'HISTORY' && (
          <section className="animate-in fade-in slide-in-from-bottom-2 duration-300 space-y-8">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
               <h3 className="text-lg font-bold text-slate-800 mb-6">Band Score Trends</h3>
               ...
               <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs">
                     <tr>
                        ...
                        <th className="px-6 py-4">Overall Band</th>
                     </tr>
                  </thead>
                           ...
                           <td className="px-6 py-4">
                              <div className="flex gap-1">
                                 {Object.values(h.sectionScores).map((s: any, i) => (
                                    <span key={i} className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs font-bold">{s.type ? s.type[0] : '?'}</span>
                                 ))}
                              </div>
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex gap-2 flex-wrap">
                                 {Object.values(h.sectionScores).map((s: any, i) => (
                                    <div key={i} className="text-xs">
                                       <span className="text-slate-400 mr-1">{s.type ? s.type.substring(0,4) : 'SEC'}:</span>
                                       <span className="font-bold text-slate-800">{s.band}</span>
                                    </div>
                                 ))}
                              </div>
                           </td>
                           <td className="px-6 py-4">
                              <span className="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-bold text-xs">Band {h.bandScore}</span>
                           </td>
                        </tr>
                     ))}
                  </tbody>
               ...
               {modalSet?.sections.map(sec => (
                  <label key={sec.id} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded cursor-pointer hover:border-blue-400 transition-colors">
                     ...
                     <div>
                        ...
                        <div className="text-xs text-slate-400 font-medium">{sec.type}</div>
                     </div>
                  </label>
               ))}
            </div>
